  ...
  return (
    <div>
      <h2 id="company-heading" data-cy="CompanyHeading">
        <Translate contentKey="secondApp.company.home.title">Companies</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="secondApp.company.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/company/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="secondApp.company.home.createLabel">Create new Company</Translate>
          </Link>
        </div>
      </h2>
        ...
        {companyList && companyList.length > 0 ? (
              ...
              {companyList.map((company, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {company.video ? (
                      <div>
                        {company.videoContentType ? (
                          <a onClick={openFile(company.videoContentType, company.video)}>
                            <Translate contentKey="entity.action.open">Open</Translate>
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {company.videoContentType}, {byteSize(company.video)}
                        </span>
                      </div>
                    ) : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="secondApp.company.home.notFound">No Companies found</Translate>
            </div>
          )
        )}
      </div>
